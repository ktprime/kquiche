...
void QuicPacketCreator::UpdatePacketNumberLength(
  ...
  if (false && packet_.packet_number_length == packet_number_length) {
    return;
  }
...
void QuicPacketCreator::SkipNPacketNumbers(
  ...
  if (DCHECK_FLAG && packet_.packet_number > packet_.packet_number + count) {
...
bool QuicPacketCreator::ConsumeDataToFillCurrentPacket(
  ...
  if (DCHECK_FLAG && GetQuicFlag(quic_enforce_single_packet_chlo) &&
...
void QuicPacketCreator::FlushCurrentPacket() {
  if (queued_frames_.empty() /* && pending_padding_bytes_ == 0***/) {
    return;
  }
...
void QuicPacketCreator::OnSerializedPacket() {
  ...
  ClearPacket();
  if (latched_hard_max_packet_length_)
    RemoveSoftMaxPacketLength();
...
void QuicPacketCreator::CreateAndSerializeStreamFrame(
  ...
  QuicDataWriter writer(sizeof(stack_buffer), encrypted_buffer);
  size_t length_field_offset = 0;
  framer_->AppendPacketHeader(header, &writer, &length_field_offset);
  ...
  bool needs_padding = plaintext_bytes_written < min_plaintext_size;
  ...
  if (needs_padding) {
    writer.WritePaddingBytes(min_plaintext_size -
...
size_t QuicPacketCreator::ExpansionOnNewFrame() const {
  ...
  if (queued_frames_.empty() || queued_frames_.back().type == ACK_FRAME) {
    return 0;
  }
...
bool QuicPacketCreator::SerializePacket(QuicOwnedPacketBuffer encrypted_buffer,
  ...
  if (packet_.encryption_level < ENCRYPTION_FORWARD_SECURE) { //TODO3
    MaybeAddPadding();
  }
...
size_t QuicPacketCreator::GetSerializedFrameLength(const QuicFrame& frame) {
  const auto packet_number_length = GetPacketNumberLength();
  ...
  if (frame_bytes >=
      MinPlaintextPacketSize(framer_->version(), packet_number_length)) {
...
QuicPacketNumber QuicPacketCreator::NextSendingPacketNumber() const {
  ...
  return QuicPacketNumber(packet_number().ToUint64() + 1);
}
...
void QuicPacketCreator::SetDefaultPeerAddress(const QuicSocketAddress& address) {
  if (false && !packet_.peer_address.IsInitialized()) {
