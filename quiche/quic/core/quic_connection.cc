...
class SendAlarmDelegate : public QuicConnectionAlarmDelegate {
 public:
  ...
  void OnAlarm() override {
    QUICHE_DCHECK(connection_->connected());
    connection_->mutable_stats().send_alarms += 1;
...
bool PacketCanReplaceServerConnectionId(const QuicPacketHeader& header,
  ...
  return header.form == IETF_QUIC_LONG_HEADER_PACKET &&
         ...
         (header.long_packet_type == INITIAL || header.long_packet_type == RETRY) &&
...
#if QUIC_TLS_SESSION
  connection_migration_use_new_cid_ = validate_client_addresses_;
#endif
...
bool QuicConnection::OnProtocolVersionMismatch(
  ...
  if (DCHECK_FLAG && perspective_ == Perspective::IS_CLIENT) {
...
bool QuicConnection::OnUnauthenticatedPublicHeader(
  ...
  if (DCHECK_FLAG && header.version_flag && header.long_packet_type == INITIAL) {
    framer_.set_drop_incoming_retry_packets(true);
  }
...
bool QuicConnection::OnStreamFrame(const QuicStreamFrame& frame) {
  ...
  if (last_received_packet_info_.decrypted_level == ENCRYPTION_INITIAL &&
    !QuicUtils::IsCryptoStreamId(transport_version(), frame.stream_id)) {
...
bool QuicConnection::OnAckFrameStart(QuicPacketNumber largest_acked,
  ...
  // A probe only contains a PING and full padding.
  if (current_effective_peer_migration_type_ != NO_CHANGE && !UpdatePacketContent(ACK_FRAME)) {
    return false;
  }
...
bool QuicConnection::OnPaddingFrame(const QuicPaddingFrame& frame) {
  QUIC_BUG_IF(quic_bug_12714_9, false)
      << "Processing PADDING frame when connection is closed. Received packet "
         "info: "
      << last_received_packet_info_;
  if (current_packet_content_ != NOT_PADDED_PING && !UpdatePacketContent(PADDING_FRAME)) {
    return false;
  }
...
void QuicConnection::OnPacketComplete() {
  ...
  ClearLastFrames();
  //CloseIfTooManyOutstandingSentPackets(); //TODO3 hybchanged check on send
}
...
void QuicConnection::MaybeRespondToConnectivityProbingOrMigration() {
  ...
  if (false && perspective_ == Perspective::IS_CLIENT) {
...
void QuicConnection::MaybeSendInResponseToPacket() {
  ...
  // connection to send.
  if (false && HandleWriteBlocked()) { //WriteIfNotBlocked will check HandleWriteBlocked
    return;
  }
...
QuicConsumedData QuicConnection::SendStreamData(QuicStreamId id,
  ...
  if (DCHECK_FLAG && write_length == 0 && state == NO_FIN) {
...
void QuicConnection::ProcessUdpPacket(const QuicSocketAddress& self_address,
  ...
  //if (!direct_peer_address_.IsInitialized()) {
    QUICHE_DCHECK(peer_address.IsInitialized() && self_address.IsInitialized());
    QUICHE_DCHECK(direct_peer_address_.IsInitialized());
    QUICHE_DCHECK(default_path_.peer_address.IsInitialized());
...
//        GetEffectivePeerAddressFromCurrentPacket();
    ...
    default_path_.peer_address = peer_address.IsInitialized()
                                     ? peer_address
  ...
  if (DCHECK_FLAG && std::abs((packet.receipt_time() - clock_->ApproximateNow()).ToSeconds()) >
      2 * 60) {
...
void QuicConnection::WriteIfNotBlocked() {
  if (DCHECK_FLAG && framer().is_processing_packet()) {
...
//      last_received_packet_info_.destination_address.IsInitialized() &&
  ...
  if (encryption_level_ == ENCRYPTION_INITIAL &&
    ...
    if (false && GetQuicFlag(quic_use_lower_server_response_mtu_for_test)) {
      SetMaxPacketLength(
...
bool QuicConnection::ValidateReceivedPacketNumber(
  ...
  return uber_received_packet_manager_.IsAwaitingPacket(
...
bool QuicConnection::CanWrite(HasRetransmittableData retransmittable) {
  ...
  QuicTime now = clock_->ApproximateNow();//TODO2: hybchanged Now()
  QuicTime::Delta delay = sent_packet_manager_.TimeUntilSend(now);
  if (/*delay.IsZero() ||**/ delay <= release_time_into_future_)
    return true;
  if (false && delay.IsInfinite()) {
//    send_alarm_->Cancel();
    return false;
  }
...
bool QuicConnection::WritePacket(SerializedPacket* packet) {
  ...
  QuicTime packet_send_time = clock_->Now();//hybchanged TODO2: CalculatePacketSentTime();
...
bool QuicConnection::ShouldDiscardPacket(EncryptionLevel encryption_level) {
  ...
  if (DCHECK_FLAG && encryption_level == ENCRYPTION_INITIAL &&
      encryption_level_ == ENCRYPTION_FORWARD_SECURE) {
...
#if QUIC_TLS_SESSION
  if (!coalescing_done_ && version().CanSendCoalescedPackets()) {
...
void QuicConnection::OnSerializedPacket(SerializedPacket& serialized_packet) {
  if (false && serialized_packet.encrypted_buffer == nullptr) {
...
//    frames.push_back(QuicFrame(stop_waiting));
  ...
  visitor_->OnAckNeedsRetransmittableFrame();//TODO3.disable it can improve performance
#endif
}
...
void QuicConnection::OnRetransmissionTimeout() {
  ...
  if (DCHECK_FLAG && retransmission_mode == QuicSentPacketManager::PTO_MODE) {
  ...
  if (!retransmission_alarm_->IsSet() && !HasQueuedData()) {
    SetRetransmissionAlarm();
  }
}
...
void QuicConnection::MaybeProcessUndecryptablePackets() {
  ...
  if (process_undecryptable_packets_alarm_->IsSet())
    process_undecryptable_packets_alarm_->Cancel();

  auto iter = undecryptable_packets_.begin();
  while (iter != undecryptable_packets_.end() && connected_) {
...
bool QuicConnection::MaybeProcessCoalescedPackets() {
  ...
  while (!received_coalesced_packets_.empty() && connected_) {
...
void QuicConnection::SendConnectionClosePacket(
  ...
  if (!SupportsMultiplePacketNumberSpaces()) {
    ...
    ScopedEncryptionLevelContext contexti(this,
  ...
  for (EncryptionLevel level :
    ...
    ScopedEncryptionLevelContext context2(this, level);
...
void QuicConnection::SetRetransmissionAlarm() {
  ...
  retransmission_alarm_->Update(sent_packet_manager_.GetRetransmissionTime(), kAlarmGranularity);
}
...
//      if (ack_timeout < ack_alarm->deadline())
    ...
    // coalescer.
#if QUIC_TLS_SESSION
    bool coalesced = connection_->version().CanSendCoalescedPackets();
    if (coalesced) {
    ...
    if (false && connection_->writer_->IsBatchMode()) {
    ...
    //QUICHE_DCHECK(connection_->connected()); //{
      //return;
    //}
#if QUIC_TLS_SESSION //line 3654  //TODO hybchanged never called
...
void QuicConnection::CheckIfApplicationLimited() {
  ...
  bool application_limited =
      !visitor_->WillingAndAbleToWrite() && buffered_packets_.empty();
...
void QuicConnection::PostProcessAfterAckFrame(bool send_stop_waiting,
  ...
  if (!packet_creator_.has_ack() && no_stop_waiting_frames_) {
    uber_received_packet_manager_.DontWaitForPacketsBefore(
        last_received_packet_info_.decrypted_level,
        SupportsMultiplePacketNumberSpaces()
...
bool QuicConnection::ShouldDetectBlackhole() const {
  if (blackhole_detection_disabled_) {
    return false;
  }
  if (GetQuicReloadableFlag(
          quic_no_path_degrading_before_handshake_confirmed) &&
...
void QuicConnection::UpdatePeerAddress(const QuicSocketAddress& peer_address) {
...
QuicConnection::PathState::PathState(PathState&& other) noexcept {
  *this = std::move(other);
}
