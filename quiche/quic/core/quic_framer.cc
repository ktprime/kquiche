...
// edge of the header.
constexpr uint8_t kPublicHeaderSequenceNumberShift = 4;
...
constexpr uint8_t kQuicFrameTypeBrokenMask = 0xE0;   // 0b 11100000
constexpr uint8_t kQuicFrameTypeSpecialMask = 0xC0;  // 0b 11000000
...
constexpr uint8_t kQuicStreamIdShift = 2;
constexpr uint8_t kQuicStreamIDLengthMask = 0x03;

// Offset may be 0, 2, 4, or 8 bytes.
constexpr uint8_t kQuicStreamShift = 3;
constexpr uint8_t kQuicStreamOffsetMask = 0x07;

// Data length may be 0 or 2 bytes.
constexpr uint8_t kQuicStreamDataLengthShift = 1;
constexpr uint8_t kQuicStreamDataLengthMask = 0x01;

// Fin bit may be set or not.
constexpr uint8_t kQuicStreamFinShift = 1;
constexpr uint8_t kQuicStreamFinMask = 0x01;
...
constexpr uint8_t kQuicSequenceNumberLengthNumBits = 2;
constexpr uint8_t kActBlockLengthOffset = 0;
constexpr uint8_t kLargestAckedOffset = 2;

// Acks may have only one ack block.
constexpr uint8_t kQuicHasMultipleAckBlocksOffset = 5;

// Timestamps are 4 bytes followed by 2 bytes.
constexpr uint8_t kQuicNumTimestampsLength = 1;
constexpr uint8_t kQuicFirstTimestampLength = 4;
constexpr uint8_t kQuicTimestampLength = 2;
// Gaps between packet numbers are 1 byte.
constexpr uint8_t kQuicTimestampPacketNumberGapLength = 1;

// Maximum length of encoded error strings.
constexpr int kMaxErrorStringLength = 256;

constexpr uint8_t kConnectionIdLengthAdjustment = 3;
constexpr uint8_t kDestinationConnectionIdLengthMask = 0xF0;
...
static bool AppendIetfConnectionIds(bool version_flag, bool use_length_prefix,
  ...
  if (use_length_prefix) {
    writer->WriteLengthPrefixedConnectionId(destination_connection_id);
    return writer->WriteLengthPrefixedConnectionId(source_connection_id);
  }
  ...
  writer->WriteUInt8(connection_id_length);
  writer->WriteConnectionId(destination_connection_id);
  if (!source_connection_id.IsEmpty())
...
QuicFramer::~QuicFramer() {
  for (int i = 0; i < NUM_ENCRYPTION_LEVELS; i++) {
    delete encrypter_[i];
    delete decrypter_[i];
  }
}
...
// static
size_t QuicFramer::GetStreamOffsetSize(uint64_t offset) {
  // 0 is a special case.
  if (offset < (1 << 16)) {
    return offset == 0 ? 0 : 2;
  }
  ...
  QUIC_BUG_IF(quic_bug_10850_12, size >= 8) << "Failed to determine StreamOffsetSize.";
  return size;
#endif
}
...
size_t QuicFramer::GetSerializedFrameLength(
  ...
  if (DCHECK_FLAG && frame.type == ACK_FRAME && frame.ack_frame == nullptr) {
...
bool QuicFramer::WriteIetfLongHeaderLength(const QuicPacketHeader& header,
  ...
  if (!header.version_flag || !QuicVersionHasLongHeaderLengths(transport_version()) ||
...
size_t QuicFramer::BuildDataPacket(const QuicPacketHeader& header,
  ...
  WriteIetfLongHeaderLength(header, &writer, length_field_offset, level);
...
std::unique_ptr<QuicEncryptedPacket> QuicFramer::BuildPublicResetPacket(
  ...
  writer.WriteBytes(reset_serialized.data(), reset_serialized.length());
...
std::unique_ptr<QuicEncryptedPacket> QuicFramer::BuildIetfStatelessResetPacket(
  ...
  writer.WriteBytes(&stateless_reset_token,
...
bool QuicFramer::ProcessPacketInternal(const QuicEncryptedPacket& packet) {
  ...
  //not support retry TODO hybchanged
  bool rv;
  if (DCHECK_FLAG && header.long_packet_type == RETRY) {
    rv = ProcessRetryPacket(&reader, header);
  } else if (header.reset_flag) {
    rv = ProcessPublicResetPacket(&reader, header);
  } else if (packet.length() <= kMaxIncomingPacketSize * 2) {
    // The optimized decryption algorithm implementations run faster when
    // operating on aligned memory.
    ABSL_CACHELINE_ALIGNED char buffer[kMaxIncomingPacketSize * 2];
...
bool QuicFramer::ProcessIetfDataPacket(QuicDataReader* encrypted_reader,
  ...
  absl::InlinedVector<char, 128> ad_storage;
  ...
  if (!DecryptPayload(packet.length(), encrypted, associated_data, *header,
    ...
    visitor_->OnUndecryptablePacket(
        ...
        has_decryption_key);
    set_detailed_error("Unable to decrypt  payload with reconstructed packet number");
    RecordDroppedPacketReason(DroppedPacketReason::DECRYPTION_FAILURE);
    return RaiseError(QUIC_DECRYPTION_FAILURE);
  }
...
bool QuicFramer::AppendPacketHeader(const QuicPacketHeader& header,
  ...
  switch (server_connection_id_included) {
    case CONNECTION_ID_ABSENT:
      writer->WriteUInt8(public_flags |
  ...
  if (header.version_flag) {
    ...
    writer->WriteUInt32(version_label);
  ...
  if (header.packet_number_length == 1)
    writer->WriteUInt8((uint8_t)header.packet_number.ToUint64());
  else
...
bool QuicFramer::AppendIetfPacketHeader(const QuicPacketHeader& header,
  ...
  AppendIetfHeaderTypeByte(header, writer);
  ...
  if (header.version_flag) {
    ...
    QuicVersionLabel version_label = CreateQuicVersionLabel(version_);
    writer->WriteUInt32(version_label);
    // Append cAppendIetfConnectionIdsonnection ID.
    AppendIetfConnectionIds(
      ...
      : EmptyQuicConnectionId(),
      writer);
  } else if (header.destination_connection_id_included != CONNECTION_ID_ABSENT) {
  ...
  if (header.packet_number_length == 1)
    writer->WriteUInt8((uint8_t)header.packet_number.ToUint64());
  else if (header.packet_number_length == 2)
  ...
  if (header.nonce != nullptr) {
    ...
    QUICHE_DCHECK_EQ(Perspective::IS_SERVER, perspective_);
    return writer->WriteBytes(header.nonce, kDiversificationNonceSize);
  }
...
uint64_t QuicFramer::CalculatePacketNumberFromWire(
  ...
  if (false && !base_packet_number.IsInitialized()) {
...
bool QuicFramer::ProcessPublicHeader(QuicDataReader* reader,
  ...
  if (public_flags > PACKET_PUBLIC_FLAGS_MAX &&
    validate_flags_ && !header->version_flag) {
  ...
  switch (public_flags & PACKET_PUBLIC_FLAGS_8BYTE_CONNECTION_ID) {
    case PACKET_PUBLIC_FLAGS_8BYTE_CONNECTION_ID:
      reader->ReadConnectionId(header_connection_id,
  ...
  if (perspective_ == Perspective::IS_CLIENT &&
    (public_flags & PACKET_PUBLIC_FLAGS_NONCE) &&
    !header->version_flag && !header->reset_flag) {
...
bool QuicFramer::ProcessUnauthenticatedHeader(QuicDataReader* encrypted_reader,
  ...
  if (DCHECK_FLAG && !visitor_->OnUnauthenticatedHeader(*header)) {
    set_detailed_error(
        "Visitor asked to stop processing of unauthenticated header.");
    return false;
  }
  ...
  if (DCHECK_FLAG && version().KnowsWhichDecrypterToUse()) {
...
bool QuicFramer::ProcessVersionLabel(QuicDataReader* reader,
                                     QuicVersionLabel* version_label) {
  return reader->ReadUInt32(version_label);
}
...
bool QuicFramer::ProcessIetfPacketHeader(QuicDataReader* reader,
  ...
  if (version_.HasLengthPrefixedConnectionIds()) {
    ...
    if (parse_result != QUIC_NO_ERROR) {
      set_detailed_error(std::string(detailed_error));
      return false;
    }
    ...
    if (0 == (header->type_byte & FLAGS_FIXED_BIT)) {
      if (!header->version_flag) {
        set_detailed_error("Fixed bit is 0 in short header.");
        return false;
      }
      else if (header->long_packet_type != VERSION_NEGOTIATION) {
        set_detailed_error("Fixed bit is 0 in long header.");
        return false;
      }
    }
  ...
  if (source_connection_id_length) {
    reader->ReadConnectionId(&header->source_connection_id, source_connection_id_length);
...
bool QuicFramer::ProcessAndCalculatePacketNumber(
    ...
    QuicPacketNumber base_packet_number, uint64_t* packet_number) {
  uint64_t wire_packet_number = 0;
  if (packet_number_length == 1)
    reader->ReadUInt8((uint8_t*)&wire_packet_number);
  else
    reader->ReadBytesToUInt64(packet_number_length, &wire_packet_number);
...
bool QuicFramer::ProcessFrameData(QuicDataReader* reader,
  ...
  if (DCHECK_FLAG && reader->IsDoneReading()) {
    ...
    switch (frame_type) {
      ...
      case WINDOW_UPDATE_FRAME: {
        ...
        if (!ProcessWindowUpdateFrame(reader, &window_update_frame)) {
          set_detailed_error("Unable to read window_update_frame stream_id.");
...
bool QuicFramer::ProcessStreamFrame(QuicDataReader* reader, uint8_t frame_type,
  ...
  uint8_t stream_id_length = (stream_flags & kQuicStreamIDLengthMask) + 1;
...
//  if (offset_length > 0) {
    offset_length += offset_length > 0 ? 1 : 0;
//  }
  ...
  auto ret = reader->ReadBytesToUInt64(offset_length, &frame->offset);

  // TODO(ianswett): Don't use absl::string_view as an intermediary.
  absl::string_view data;
  if (has_data_length) {
    ret = reader->ReadStringPiece16(&data);
  } else {
    ret = reader->ReadStringPiece(&data, reader->BytesRemaining());
  }
...
bool QuicFramer::ProcessAckFrameBlocks(QuicDataReader * reader,
  size_t num_ack_blocks, uint64_t& first_received, QuicPacketNumberLength ack_block_length)
{
  for (size_t i = 0; i < num_ack_blocks; ++i) {
    uint8_t gap = 0;
    reader->ReadUInt8(&gap);
    uint64_t current_block_length = 0;
    reader->ReadBytesToUInt64(ack_block_length, &current_block_length);
...
bool QuicFramer::ProcessAckFrame(QuicDataReader* reader, uint8_t frame_type) {
  ...
  if (largest_acked_length == 2)
    reader->ReadUInt16((uint16_t*)&largest_acked);
  else
    reader->ReadBytesToUInt64(largest_acked_length, &largest_acked);

  if (DCHECK_FLAG && first_sending_packet_number_.ToUint64() > largest_acked) {
  ...
  reader->ReadUFloat16(&ack_delay_time_us);
  //update rtt and check largest_acked, ack_time
...
//  QUICHE_DCHECK(ack_block_length == 1);
  if (ack_block_length == 1)
    reader->ReadUInt8((uint8_t*)&first_block_length);
  else
...
//  bool first_ack_block_underflow = first_block_length > largest_acked + 1;
  else if (DCHECK_FLAG && first_block_length /* + first_sending_packet_number_.ToUint64() */ >
      largest_acked) {
    set_detailed_error("Underflow with first ack block length largest acked");
    return false;
  }

  //add valid packet number to sentmgr.packets_acked_
  uint64_t first_received = largest_acked + 1 - first_block_length;
  visitor_->OnAckRange(QuicPacketNumber(first_received),
  ...
  if (num_ack_blocks > 0 && !ProcessAckFrameBlocks(reader, num_ack_blocks, first_received, ack_block_length)) {
    return false;
  }

  reader->ReadUInt8(&num_received_packets);

  if (num_received_packets && !ProcessTimestampsInAckFrame(num_received_packets, QuicPacketNumber(largest_acked), reader)) {
    return false;
  }
...
bool QuicFramer::ProcessTimestampsInAckFrame(uint8_t num_received_packets,
  ...
  QUICHE_DCHECK(num_received_packets > 0);
  uint8_t delta_from_largest_observed;
  reader->ReadUInt8(&delta_from_largest_observed);
...
bool QuicFramer::ProcessRstStreamFrame(QuicDataReader* reader,
                                       QuicRstStreamFrame* frame) {
  reader->ReadUInt32(&frame->stream_id);
...
bool QuicFramer::ProcessWindowUpdateFrame(QuicDataReader* reader,
                                          QuicWindowUpdateFrame* frame) {
  reader->ReadUInt32(&frame->stream_id);
  return reader->ReadUInt64(&frame->max_data);
}
...
static bool memvcmp(const char* memory, unsigned char val, unsigned int size)
{
    const unsigned char* mm = (unsigned char*)memory;
...
void QuicFramer::ProcessPaddingFrame(QuicDataReader* reader,
  ...
  //uint8_t next_byte;
#if 1
  const auto remains = reader->BytesRemaining();
  if (remains && memvcmp(reader->data() + reader->pos(), 0, remains)) {
    reader->AdvancePos(remains);
    frame->num_padding_bytes += remains;
    //return;
  }
#endif

  while (reader->BytesRemaining() && reader->PeekByte() == 0x00) {
...
void QuicFramer::SetDecrypter(EncryptionLevel level,
  ...
  delete decrypter_[decrypter_level_];
  decrypter_[decrypter_level_] = nullptr;
  delete decrypter_[level];
  decrypter_[level] = decrypter.release();
...
//  QUICHE_DCHECK(!version_.KnowsWhichDecrypterToUse());
  ...
  if (alternative_decrypter_level_ != NUM_ENCRYPTION_LEVELS) {
    delete decrypter_[alternative_decrypter_level_];
...
bool QuicFramer::DoKeyUpdate(KeyUpdateReason reason) {
  ...
  if (!next_decrypter_) {
    ...
    // yet.
    next_decrypter_ = visitor_->AdvanceKeysAndCreateCurrentOneRttDecrypter().release();
  }
  ...
  previous_decrypter_ = decrypter_[ENCRYPTION_FORWARD_SECURE];
  decrypter_[ENCRYPTION_FORWARD_SECURE] = next_decrypter_;
  delete encrypter_[ENCRYPTION_FORWARD_SECURE];
  encrypter_[ENCRYPTION_FORWARD_SECURE] = next_encrypter.release();
  switch (reason) {
...
const QuicDecrypter* QuicFramer::GetDecrypter(EncryptionLevel level) const {
  QUICHE_DCHECK(version_.KnowsWhichDecrypterToUse());
  return decrypter_[level];
}

const QuicDecrypter* QuicFramer::decrypter() const {
  return decrypter_[decrypter_level_];
}
...
const QuicDecrypter* QuicFramer::alternative_decrypter() const {
  ...
  return decrypter_[alternative_decrypter_level_];
}
...
void QuicFramer::SetEncrypter(EncryptionLevel level,
  ...
  delete encrypter_[level];
  encrypter_[level] = encrypter.release();
}
...
void QuicFramer::SetInitialObfuscators(QuicConnectionId connection_id) {
  ...
  CryptoUtils::CreateInitialObfuscators(perspective_, version_, connection_id,
                                        &crypters);
  encrypter_[ENCRYPTION_INITIAL] = crypters.encrypter.release();
  decrypter_[ENCRYPTION_INITIAL] = crypters.decrypter.release();
}
...
bool QuicFramer::ApplyHeaderProtection(EncryptionLevel level, char* buffer,
...
#else
  char mask[32 * 2] = {0};
  auto mask_size = encrypter_[level]->GenerateHeaderProtectionMask(sample, mask);
  if (DCHECK_FLAG && mask_size == 0) {
    QUIC_BUG(quic_bug_10850_61) << "Unable to generate header protection mask.";
    return false;
  }
  QuicDataReader mask_reader(mask, mask_size);
#endif
  ...
  uint8_t mask_byte;
  mask_reader.ReadUInt8(&mask_byte);
  ...
  for (size_t i = 0; i < last_written_packet_number_length_; ++i) {
    ...
    mask_reader.ReadUInt8(&pn_mask_byte);
...
bool QuicFramer::RemoveHeaderProtection(QuicDataReader* reader,
  ...
  QuicDecrypter* decrypter = decrypter_[expected_decryption_level];
  ...
  // The sample starts 4 bytes after the start of the packet number.
  absl::string_view pn;
  sample_reader.ReadStringPiece(&pn, 4);
  ...
  if (mask.empty()) {
#else
  char mask[kDiversificationNonceSize * 2] = {0};
  auto mask_size = decrypter->GenerateHeaderProtectionMask(&sample_reader, mask);
  QuicDataReader mask_reader(mask, mask_size);
  if (mask_size == 0) {
#endif
  ...
  ad_writer.WriteUInt8(header->type_byte);
...
bool QuicFramer::DecryptPayload(size_t udp_packet_length,
  ...
  QuicDecrypter* decrypter = decrypter_[level];
  ...
  if (version().KnowsWhichDecrypterToUse()) {
    ...
    decrypter = decrypter_[level];
    if (DCHECK_FLAG && decrypter == nullptr) {
      return false;
    }
    ...
    if (support_key_update_for_connection_ &&
      ...
      if (key_phase != current_key_phase_bit_) {
        if ((current_key_phase_first_received_packet_number_.IsInitialized() &&
          ...
          if (!next_decrypter_) {
            next_decrypter_ =
                visitor_->AdvanceKeysAndCreateCurrentOneRttDecrypter().release();
            if (!next_decrypter_) {
    ...
    if (DCHECK_FLAG && !EncryptionLevelIsValid(alternative_decrypter_level_)) {
      QUIC_BUG(quic_bug_10850_71)
          << "Attempted to decrypt with bad alternative_decrypter_level_";
      return false;
    }
    alternative_decrypter = decrypter_[alternative_decrypter_level_];
  }
...
size_t QuicFramer::GetAckFrameSize(
  ...
  ack_size = GetMinAckFrameSize(version_.transport_version, ack,
                                local_ack_delay_exponent_,
                                process_timestamps_ && UseIetfAckWithReceiveTimestamp(ack));
...
bool QuicFramer::AppendStreamId(size_t stream_id_length, QuicStreamId stream_id,
                                QuicDataWriter* writer) {
  if (DCHECK_FLAG && (stream_id_length != 1)) {
    QUIC_BUG(quic_bug_10850_77)
        << "Invalid stream_id_length: " << stream_id_length;
    return false;
  }
  return writer->WriteUInt8(stream_id);
...
bool QuicFramer::AppendStreamOffset(size_t offset_length,
                                    QuicStreamOffset offset,
                                    QuicDataWriter* writer) {
  if (DCHECK_FLAG && (offset_length == 1 || offset_length > 8)) {
...
bool QuicFramer::AppendAckBlock(uint8_t gap,
  ...
  writer->WriteUInt8(gap);
  return AppendPacketNumber(length_length, QuicPacketNumber(length), writer);
}
...
bool QuicFramer::AppendStreamFrame(const QuicStreamFrame& frame,
  ...
  if (true || data_producer_ != nullptr) {
    ...
    data_producer_->WriteStreamData(frame.stream_id, frame.offset, frame.data_length, writer);
    return true;
  }

  return writer->WriteBytes(frame.data_buffer, frame.data_length);
}
...
bool QuicFramer::AppendAckFrameAndTypeByte(const QuicAckFrame& frame,
  ...
  int32_t available_timestamp_and_ack_block_bytes =
      ...
      GetMinAckFrameSize(version_.transport_version, frame,
                         local_ack_delay_exponent_,
                         process_timestamps_ && UseIetfAckWithReceiveTimestamp(frame)) -
  ...
  // Largest acked.
  AppendPacketNumber(largest_acked_length, largest_acked, writer);
  ...
  writer->WriteUFloat16(ack_delay_time_us);

  if (num_ack_blocks > 0) {
    writer->WriteUInt8(num_ack_blocks);
  }
...
//      QUICHE_DCHECK(num_encoded_gaps == 1);
  ...
  if (DCHECK_FLAG && process_timestamps_ && writer->capacity() - writer->length() >=
    ...
    uint8_t num_received_packets = 0;
    writer->WriteUInt8(num_received_packets);
  }
...
bool QuicFramer::AppendTimestampsToAckFrame(const QuicAckFrame& frame,
  ...
  writer->WriteBytes(&num_received_packets, 1);
...
bool QuicFramer::AppendRstStreamFrame(const QuicRstStreamFrame& frame,
  ...
  writer->WriteUInt32(frame.stream_id);
  writer->WriteUInt64(frame.byte_offset);

  uint32_t error_code = static_cast<uint32_t>(frame.error_code);
  return writer->WriteUInt32(error_code);
}
...
bool QuicFramer::AppendConnectionCloseFrame(
  ...
  return writer->WriteStringPiece16(TruncateErrorString(frame.error_details));
}
...
bool QuicFramer::AppendWindowUpdateFrame(const QuicWindowUpdateFrame& frame,
  ...
  writer->WriteUInt32(stream_id);
  return writer->WriteUInt64(frame.max_data);
}
...
bool QuicFramer::AppendBlockedFrame(const QuicBlockedFrame& frame,
  ...
  uint32_t stream_id = static_cast<uint32_t>(frame.stream_id);
  return writer->WriteUInt32(stream_id);
}
...
QuicErrorCode QuicFramer::ParsePublicHeaderDispatcherShortHeaderLengthUnknown(
    ...
    QuicConnectionId* destination_connection_id,
    QuicConnectionId* source_connection_id,
    absl::optional<absl::string_view>* retry_token, std::string_view* detailed_error,
...
inline bool ParseLongHeaderConnectionIds(
    ...
    QuicConnectionId& source_connection_id, std::string_view& detailed_error) {
