...
void SmallVectorTemplateBase<T, isPodLike>::grow(size_t MinSize) {
  ...
  size_t NewCapacity = size_t(llvm_vecsmall::detail::NextPowerOf2(CurCapacity+1));
...
public:
  ...
  iterator emplace(const_iterator it, ArgTypes &&... Args) {
    return insert((iterator)it, std::forward<ArgTypes>(Args)...);
  }
  ...
  void set_size(const size_type N) {
...
*/
  ...
  const SmallVector &operator=(SmallVector &&RHS) noexcept {
    SmallVectorImpl<T>::operator=(::std::move(RHS));
    return *this;
  }
  ...
  const SmallVector &operator=(SmallVectorImpl<T> &&RHS) noexcept {
    SmallVectorImpl<T>::operator=(::std::move(RHS));
    return *this;
  }
