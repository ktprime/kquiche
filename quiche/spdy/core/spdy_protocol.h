...
// flow control).
constexpr SpdyStreamId kSessionFlowControlStreamId = 0;

// 0 is not a valid stream ID for any other purpose than flow control.
constexpr SpdyStreamId kInvalidStreamId = 0;

// Max stream id.
constexpr SpdyStreamId kMaxStreamId = 0x7fffffff;

// The maximum possible frame payload size allowed by the spec.
constexpr uint32_t kSpdyMaxFrameSizeLimit = (1 << 24) - 1;

// The initial value for the maximum frame payload size as per the spec. This is
// the maximum control frame size we accept.
constexpr uint32_t kHttp2DefaultFramePayloadLimit = 1 << 14;
...
constexpr size_t kHttp2MaxControlFrameSendSize = kHttp2DefaultFramePayloadLimit - 1;
...
// The initial value for the maximum frame payload size as per the spec. This is
// the maximum control frame size we accept.
constexpr uint32_t kHttp2DefaultFrameSizeLimit =
...
constexpr uint32_t kSpdyInitialHeaderListSizeLimit = 0xFFFFFFFF;

// Maximum window size for a Spdy stream or session.
constexpr int32_t kSpdyMaximumWindowSize = 0x7FFFFFFF;  // Max signed 32bit int

// Maximum padding size in octets for one DATA or HEADERS or PUSH_PROMISE frame.
constexpr int32_t kPaddingSizePerFrame = 256;
...
constexpr int kHttp2ConnectionHeaderPrefixSize = 24;
...
// https://www.chromium.org/spdy/spdy-protocol/spdy-protocol-draft3-1#TOC-2.3.3-Stream-priority
constexpr SpdyPriority kV3HighestPriority = 2;
constexpr SpdyPriority kV3LowestPriority = 4;
...
constexpr int kHttp2MinStreamWeight = 1;
constexpr int kHttp2MaxStreamWeight = 256;
constexpr int kHttp2DefaultStreamWeight = 16;
...
constexpr unsigned int kHttp2RootStreamId = 0;
...
constexpr size_t kDataFrameMinimumSize = kFrameHeaderSize;
constexpr size_t kHeadersFrameMinimumSize = kFrameHeaderSize;
// PRIORITY frame has stream_dependency (4 octets) and weight (1 octet) fields.
constexpr size_t kPriorityFrameSize = kFrameHeaderSize + 5;
// RST_STREAM frame has error_code (4 octets) field.
constexpr size_t kRstStreamFrameSize = kFrameHeaderSize + 4;
constexpr size_t kSettingsFrameMinimumSize = kFrameHeaderSize;
constexpr size_t kSettingsOneSettingSize =
    sizeof(uint32_t) + sizeof(SpdySettingsId);
// PUSH_PROMISE frame has promised_stream_id (4 octet) field.
constexpr size_t kPushPromiseFrameMinimumSize = kFrameHeaderSize + 4;
// PING frame has opaque_bytes (8 octet) field.
constexpr size_t kPingFrameSize = kFrameHeaderSize + 8;
// GOAWAY frame has last_stream_id (4 octet) and error_code (4 octet) fields.
constexpr size_t kGoawayFrameMinimumSize = kFrameHeaderSize + 8;
// WINDOW_UPDATE frame has window_size_increment (4 octet) field.
constexpr size_t kWindowUpdateFrameSize = kFrameHeaderSize + 4;
constexpr size_t kContinuationFrameMinimumSize = kFrameHeaderSize;
// ALTSVC frame has origin_len (2 octets) field.
constexpr size_t kGetAltSvcFrameMinimumSize = kFrameHeaderSize + 2;
// PRIORITY_UPDATE frame has prioritized_stream_id (4 octets) field.
constexpr size_t kPriorityUpdateFrameMinimumSize = kFrameHeaderSize + 4;
...
// length.
constexpr size_t kAcceptChFramePerEntryOverhead = 4;
...
constexpr size_t kSizeOfSizeField = sizeof(uint32_t);
// Initial window size for a stream in bytes.
constexpr int32_t kInitialStreamWindowSize = 64 * 1024 - 1;
// Initial window size for a session in bytes.
constexpr int32_t kInitialSessionWindowSize = 64 * 1024 - 1;
...
constexpr size_t kPerHeaderHpackOverhead = 4;
